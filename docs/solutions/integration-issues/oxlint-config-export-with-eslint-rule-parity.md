...
> entirely (typescript-eslint caps TypeScript at <6.1.0, blocking TypeScript 7) and the
> package was renamed to `oxlint-config-decent`. The `./oxlint` subpath still exists as a
> compatibility alias. Do not restore the ESLint export.
